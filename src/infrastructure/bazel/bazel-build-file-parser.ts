...
import type { BazelPackageInfo, BazelParseResult, BazelScheme, BazelTarget, BazelXcodeConfiguration } from "../../domain/entities/bazel/types.js";

// Re-export types for convenience
export type { BazelPackageInfo, BazelParseResult, BazelScheme, BazelTarget, BazelXcodeConfiguration } from "../../domain/entities/bazel/types.js";

/**
 * Legacy BUILD file parser
 * This parser reads and parses BUILD/BUILD.bazel files directly
 * Used for backward compatibility with existing code
 * 
 * For new code, prefer using BazelParser with bazel query
 */
export class BazelBuildFileParser {
  /**
  ...
  static parse(content: string, filePath?: string): BazelParseResult {
    ...
    result.xcschemes = BazelBuildFileParser.parseSchemes(content);
    result.xcode_configurations = BazelBuildFileParser.parseXcodeConfigurations(content);

    const { targets, testTargets } = BazelBuildFileParser.parseTargets(content, filePath);
  ...
  private static parseTargets(
    ...
    BazelBuildFileParser.parseDdIosPackageTargets(content, filePath, targets, testTargets);

    // Parse cx_module targets
    BazelBuildFileParser.parseCxModuleTargets(content, filePath, targets, testTargets);

    // Parse swift_library targets
    BazelBuildFileParser.parseSwiftLibraryTargets(content, filePath, targets);

    // Parse dd_ios_application targets
    BazelBuildFileParser.parseDdIosApplicationTargets(content, filePath, targets);

    // Parse top_level_target entries from xcodeproj
    BazelBuildFileParser.parseTopLevelTargets(content, filePath, targets, testTargets);
  ...
  private static parseDdIosPackageTargets(
    ...
    while ((packageStartMatch = ddIosPackageStartRegex.exec(content)) !== null) {
      ...
      while ((libraryMatch = libraryRegex.exec(targetsContent)) !== null) {
        ...
        const target = BazelBuildFileParser.parseTargetParams(targetParams, "library", packageName, filePath);
      ...
      while ((testMatch = testRegex.exec(targetsContent)) !== null) {
        ...
        const target = BazelBuildFileParser.parseTargetParams(targetParams, "test", packageName, filePath);
      ...
      while ((binaryMatch = binaryRegex.exec(targetsContent)) !== null) {
        ...
        const target = BazelBuildFileParser.parseTargetParams(targetParams, "binary", packageName, filePath);
        if (target) targets.push(target);
      }
    }
  }
  ...
  private static parseCxModuleTargets(
    ...
    while ((moduleMatch = cxModuleRegex.exec(content)) !== null) {
      ...
      const packagePath = BazelBuildFileParser.getPackagePath(filePath);
  ...
  private static getPackagePath(filePath?: string): string {
    ...
    if (dir.includes("/Sources/")) {
      // For package-style structure, try to get the package name before /Sources/
  ...
  static parsePackage(buildFileContent: string, packagePath: string): BazelPackageInfo {
    ...
    const parseResult = BazelBuildFileParser.parse(buildFileContent, path.join(packagePath, "BUILD.bazel"));
