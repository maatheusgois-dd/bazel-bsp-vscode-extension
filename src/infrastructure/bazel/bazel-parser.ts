...
export class BazelParser {
  ...
  static parse(content: string, filePath?: string): BazelParseResult {
    ...
    result.xcschemes = BazelParser.parseSchemes(content);
    result.xcode_configurations = BazelParser.parseXcodeConfigurations(content);

    const { targets, testTargets } = BazelParser.parseTargets(content, filePath);
  ...
  private static parseTargets(
    ...
    BazelParser.parseDdIosPackageTargets(content, filePath, targets, testTargets);

    // Parse cx_module targets
    BazelParser.parseCxModuleTargets(content, filePath, targets, testTargets);

    // Parse swift_library targets
    BazelParser.parseSwiftLibraryTargets(content, filePath, targets);

    // Parse dd_ios_application targets
    BazelParser.parseDdIosApplicationTargets(content, filePath, targets);

    // Parse top_level_target entries from xcodeproj
    BazelParser.parseTopLevelTargets(content, filePath, targets, testTargets);
  ...
  private static parseDdIosPackageTargets(
    ...
    while ((packageStartMatch = ddIosPackageStartRegex.exec(content)) !== null) {
      ...
      while ((libraryMatch = libraryRegex.exec(targetsContent)) !== null) {
        ...
        const target = BazelParser.parseTargetParams(targetParams, "library", packageName, filePath);
      ...
      while ((testMatch = testRegex.exec(targetsContent)) !== null) {
        ...
        const target = BazelParser.parseTargetParams(targetParams, "test", packageName, filePath);
      ...
      while ((binaryMatch = binaryRegex.exec(targetsContent)) !== null) {
        ...
        const target = BazelParser.parseTargetParams(targetParams, "binary", packageName, filePath);
        if (target) targets.push(target);
      }
    }
  }
  ...
  private static parseCxModuleTargets(
    ...
    while ((moduleMatch = cxModuleRegex.exec(content)) !== null) {
      ...
      const packagePath = BazelParser.getPackagePath(filePath);
  ...
  static parsePackage(buildFileContent: string, packagePath: string): BazelPackageInfo {
    ...
    const parseResult = BazelParser.parse(buildFileContent, path.join(packagePath, "BUILD.bazel"));
